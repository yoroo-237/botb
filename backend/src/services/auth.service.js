...
export const authService = {
  async register({ username, password }) {
    if (!username || username.trim().length < 3) throw appError('Username trop court (min 3 caractères)', 400);
  ...
  async login({ username, password }) {
    if (!username || !password) throw appError('Username et password requis', 400);

    const user = await prisma.user.findUnique({ where: { username } });
    if (!user)          throw appError('Identifiants invalides', 401);
    if (!user.isActive) throw appError('Compte désactivé', 403);

    const match = await bcrypt.compare(password, user.passwordHash);
    if (!match) throw appError('Identifiants invalides', 401);
  ...
  async refresh(refreshToken) {
    if (!refreshToken) throw appError('refreshToken requis', 400);

    let payload;
    try {
      payload = verifyRefreshToken(refreshToken);
    } catch {
      throw appError('Refresh token invalide ou expiré', 401);
    }

    const user = await prisma.user.findUnique({ where: { id: payload.sub } });
    if (!user || !user.isActive) throw appError('Utilisateur introuvable ou désactivé', 401);
